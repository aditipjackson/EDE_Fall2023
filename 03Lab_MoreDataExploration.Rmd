...
matrix1[4, ] #  
matrix1[ , 3] #
matrix1[c(12, 14)] # want elements in index 12 and index 14 of matrix
matrix1[c(12:14)] # want elements in indeces 12 though 14 of matrix
matrix1[2:4, 1:3] # 
...
> Answer: Matrices and dataframes show up as data; vectors and variables show up as values; functions are their own category

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: You can access specific elements of the data frame using the $
...
```{r DataExercise-1a}
# 10 High Temp in Durham (F)
High_temp_vector_Durham_F <- c(83,83,83,78,71,79,82,85,84,82)

# 10 Low Temp in Durham (F)
Low_temp_vector_Durham_F <- c(56,58,60,61,63,63,64,64,63,60)
...
```{r DataExercise-1b}
# 10 High Temp in Durham (C)
high_C <- (High_temp_vector_Durham_F - 32)*5/9 # could also have just created a function


# 10 Low Temp in Durham (C)
low_C <- (Low_temp_vector_Durham_F-32)*5/9
...
df_temp <- data.frame("High TEMP IN f" = High_temp_vector_Durham_F, Low_temp_vector_Durham, high_C,low_C)
...
date_juneteenth # prints in reverse as default
```
...
str_past <- "55-feb-3"
#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=23L) # cutoff_2000 is name of the argument, 23L is what to used as cutoff... Next year would be 24L.... Right now any 24 that appears in the data set is probably historical
date_obj_past
