...
head(NTL.phys.data) # prints first few rows
summary(NTL.phys.data) # gives summary for each of the columns
str(NTL.phys.data) # gives us data types
dim(NTL.phys.data)

class(NTL.phys.data$sampledate)
# Format sampledate as date
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") # parsing this character into a date object
class(NTL.phys.data$sampledate) # now it's a date
```
...
`==` # used for comparing two sides of an equality 
`!=` # checking if variables are different 
`<`
`<=`
`>`
`>=`
`&` # use & if comparing multiple variables
`|` # use | for or / other
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # filtering rows with 0 in column depth
...
# Choose multiple conditions to filter (only chooses rows)
...
# allows us to choose columns in our data setn (creating subset of data)
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) # this is better approach than line 172
# interval can allow you to select start and end date
```
