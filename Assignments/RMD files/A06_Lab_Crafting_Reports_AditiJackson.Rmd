...
# added "message=FALSE" so that no messages show up in knitted doc
# added "error=FALSE" so that no errors show up in knitted doc
# added "results='hide'" so that code output is not displayed in knitted doc

#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr);library(ggplot2)
...
# added "echo = FALSE" to code chunk options so that only output is displayed
...
## Table of mean PM2.5 concentrations for each county
kable(summary_PM25,
      col.names = c("County","$\\mu g/m^3$"),
      caption = "Mean Particulates (2.5mm)")

## Table of mean ozone concentrations for each county
kable(summary_Ozone,
      col.names = c("County","$\\mu g/m^3$"),
      caption = "Mean Ozone")
...
```{r 2018 plot, echo=FALSE,warning=FALSE,fig.align='left',fig.height=3,fig.width=4,fig.cap="Monthly Ozone, 2018"}

# added "echo=FALSE", "warning=FALSE", "fig.align='left'","fig.height=3='left'"
## "fig.width=4='left'", "fig_cap="Monthly Ozone, 2018" 


epa_data_2018 <- filter(epa_data,Year=="2018")

ggplot(epa_data_2018,aes(
  x=factor(epa_data_2018$Month,levels=1:12,labels=month.abb),
  y=Ozone))+
  geom_boxplot()+
  labs(x="Month",y="Concentration ($\\mu g/m^3$)",title="Recorded Ozone by Month (2018)")
  

```

```{r 2019 plot,echo=FALSE,warning=FALSE,fig.align='left',fig.height=3,fig.width=4,fig.cap="Monthly Ozone, 2019"}

# added "echo=FALSE", "warning=FALSE", "fig.align='left'","fig.height=3='left'"
## "fig.width=4='left'", "fig_cap="Monthly Ozone, 2019" 

epa_data_2019 <- filter(epa_data,Year=="2019")

ggplot(epa_data_2019,aes(
  x=factor(epa_data_2019$Month,levels=1:12,labels=month.abb),
  y=Ozone))+
  geom_boxplot()+
  labs(x="Month",y="Concentration ($\\mu g/m^3$)",title="Recorded Ozone by Month (2019)")
