...
ggplot(EPAair,aes(x=Ozone,fill=as.factor(Year)))+
  # with density we don't need to specify a y variable b/c it's going to be frequency
...
ggplot(EPAair,aes(x=as.factor(Year),
                  y=Ozone))+
  geom_violin(draw_quantiles = 0.5)+ # draws the straight lines in the middle of the plots
  stat_summary(geom="text",
               fun=max,
               vjust=1,
               size=4,
