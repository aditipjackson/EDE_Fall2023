...
Rainfall_BR_processed <-
  Rainfall_BR %>% 
  rename( Rainfall = Rainfall....MM.) %>%  # renmaing first column
  separate(Statistics,c("Null","Month","Null2")," ") %>% # separate (it's like text to columns in Excel); separate is going to be the space character; notice also a space in front of the January character
  mutate( Date = my(paste0(Month,"-",Year))) %>% # can put month and date together by creating a new column and using lubridate
  select(Date,Rainfall) # b/c day is not specified, it'll give 1st of each month (default)
...
## create a sequence that starts from first observation and ends at last observation, specify month (you want your data set to be equally spaced by month); the sequence will be a data frame
## then use left join function to put together data set with sequence data frame
## it will populate with N/As have a missing rainfall or temperature value
...
ggplot(BR_complete,aes(x = Date,y = Rainfall))+
  ...
  labs(x = "", y = "Rainfall")
# you can tell by the oscillation feature of the graph that there is seasonality in the data set (rainfall = seasonal)
...
BR_rainfall_decomp <- stl(rainfall_ts, s.window = "periodic")
...
# the bigger the grey bar, the less of an imapct that component has on your data set
...
BR_rainfall_components <- 
  as.data.frame(BR_rainfall_decomp$time.series[ ,1:3])

BR_rainfall_components <-  
  mutate(BR_rainfall_components,
        Observed = BR_,     
